...
app.post('/events', async (req, res) => {
  try {
    const { title, description, event_date, start_time, end_time, 
            ...
            organizer_name, contact_email, max_capacity, created_by,
            category_id, location_id } = req.body;
    
    if (!title || !event_date || !created_by) {
      return res.status(400).json({ error: 'Missing required fields' });
    }
    
    // Use provided category_id or get/create by name
    ...
    // Use provided location_id or get/create by name
...
app.put('/events/:id', async (req, res) => {
  try {
    const { user_id, user_role, title, description, event_date, start_time, end_time, 
            location_name, building_name, category_name, organizer_name, contact_email, max_capacity,
            category_id, location_id } = req.body;
...
app.delete('/comments/:comment_id', async (req, res) => {
  try {
    ...
    if (!isOwner && !isAdmin) return res.status(403).json({ error: 'Not authorized to delete this comment' });
...
app.delete('/photos/:photo_id', async (req, res) => {
  try {
    ...
    const isAdmin = user_role === 'super_admin' || user_role === 'it_admin';
    if (!isOwner && !isAdmin) return res.status(403).json({ error: 'Not authorized to delete this photo' });
...
app.get('/saved/user/:user_id', async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT e.*, c.category_name, l.location_name
       FROM saved_events se
       JOIN events e ON se.event_id = e.event_id
       LEFT JOIN categories c ON e.category_id = c.category_id
       LEFT JOIN locations l ON e.location_id = l.location_id
       WHERE se.user_id = $1 AND e.is_active = TRUE
       ORDER BY e.event_date ASC`,
      [req.params.user_id]
    );
    res.json(result.rows);
